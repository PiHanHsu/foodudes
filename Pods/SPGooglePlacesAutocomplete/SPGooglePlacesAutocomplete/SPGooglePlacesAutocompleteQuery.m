...
@interface SPGooglePlacesAutocompleteQuery()<CLLocationManagerDelegate>
{
...
- (id)initWithApiKey:(NSString *)apiKey {
    ...
    if (self) {
        ...
        self.radius = 500;
        self.types = SPPlaceTypeEstablishment;
    }
    return self;
}
...
- (NSString *)googleURLString {
    ...
    NSMutableString *url = [NSMutableString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&sensor=%@&key=%@&location=%f,%f&language=zh-TW",
                            [self.input stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
                            SPBooleanStringForBool(self.sensor), self.key, manager.location.coordinate.latitude, manager.location.coordinate.longitude];
//    NSMutableString *url = [NSMutableString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/nearbysearch/json?name=%@&sensor=%@&key=%@&location=%f,%f&language=zh-TW",
//                            [self.input stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
//                            SPBooleanStringForBool(self.sensor), self.key, manager.location.coordinate.latitude, manager.location.coordinate.longitude];
...
//        [url appendFormat:@"&location=%f,%f", manager.location.coordinate.latitude, manager.location.coordinate.longitude];
//        NSLog(@"lat: %.6f, lng: %.6f",manager.location.coordinate.latitude, manager.location.coordinate.longitude );
//    }
